...
class Passaro(Ator):
    ...
    def calcular_posicao(self, tempo):
        ...
        if self._esta_voando():
            delta_t = abs(tempo - self._tempo_de_lancamento)
            self._lancamento_posicao_vertical(delta_t)
            self._lancamento_posicao_horizontal(delta_t)
    ...
    def _lancamento_posicao_vertical(self, delta_t):
        y_atual = self._y_inicial
        y_atual += self.velocidade_escalar * math.sin(self._angulo_de_lancamento) * delta_t
        y_atual -= (GRAVIDADE * (delta_t ** 2))/ 2
        self.y = y_atual

    def _lancamento_posicao_horizontal(self, delta_t):
